...
import com.example.nooneschool.home.adapter.AdapterComment;
...
public class CommentActivity extends Activity implements ListItemClickHelp {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list = new ArrayList<>();
		adapter = new AdapterComment(CommentActivity.this, list, callback);
	...
	public void getMealData(final View item) {
		...
		getCommentDataRunnable = new Runnable() {
			...
			@Override
			public void run() {
				final String result = HomeService.HomeServiceByPost(list.size(), id,
						"NoOneService/GetCommentServlet?");
						...
						if (result != null && !result.equals("[]")) {
							try {
								...
								for (int i = 0; i < ja.length(); i++) {
									...
									list.add(new ListComment(id, name, content, time, imgurl));
								}
