...
import com.zh.Dao.common.DaoException;
import com.zh.common.DBHelper;
import com.zh.entity.common.Entity;
import com.zh.entity.common.Page;

/**
 * 使用commons-dbutils.jar实现的�?用DAO实现�?
 * 
 * @param <T>
 */
...
public abstract class BaseDaoImpl<T extends Entity> implements BaseDao<T> {
	...
	public T save(T entity) throws DaoException {
		...
		for (Map.Entry<String, Object> entry : map.entrySet()) {
			...
			String name = entry.getKey(); // 字段�?
			sql.append(name);

			values.append("?");
			Object value = entry.getValue(); // 字段的�?
	...
	/**
	 * 通过反射机制，获取指定实体类对象的所有属性名及对应�?的Map实例
	 * 
	 * @param entity
	 *            实例
	...
	protected Map<String, Object> getFieldValueMap(T entity) {
		...
		for (int i = 0; i < fields.length; i++) {
			Field f = fields[i];
			String name = f.getName(); // 属�?�?

			if (!"serialVersionUID".equalsIgnoreCase(name)) { // 忽略序列化版本ID�?
				// System.out.println("字段的名�? + name);

				f.setAccessible(true);// 取消 Java语言访问�?��
				try {
					Object value = f.get(entity); // 反射获取该属性的�?
