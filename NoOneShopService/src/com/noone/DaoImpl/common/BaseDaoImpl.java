package com.noone.DaoImpl.common;
...
import com.noone.common.DBHelper;
import com.noone.entity.common.Entity;
import com.noone.entity.common.Page;
...
public abstract class BaseDaoImpl<T extends Entity> implements BaseDao<T> {
	...
	public String update(T entity) throws DaoException {
		...
		try {
			conn = DBHelper.getConn();
			result = qr.update(conn, sql.toString(), paramValues.toArray());
		} catch (SQLException e) {
			return "200";
		} finally {
			DbUtils.closeQuietly(conn);
	...
	public String delete(Long id) throws DaoException {
		...
		try {
			conn = DBHelper.getConn();
			result = qr.update(conn, sql.toString(), id);
		} catch (SQLException e) {
			return "200";
		} finally {
			DbUtils.closeQuietly(conn);
	...
	public String update(String sql, Object... paramValues) throws DaoException {
		int result;
		Connection conn = null;
		try {
			conn = DBHelper.getConn();

			result = qr.update(conn, sql, paramValues);
		} catch (SQLException e) {
			return "200";
		} finally {
			DbUtils.closeQuietly(conn);
